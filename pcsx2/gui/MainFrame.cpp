...

{
	...
	if( PCSX2_isReleaseVersion )
	{
		...
		wintitle.Printf( L"%s-execmon  %d.%d.%d", pxGetAppName().c_str(), PCSX2_VersionHi, PCSX2_VersionMid, PCSX2_VersionLo );
	}
	else
	{
		// beta / development editions, which feature revision number and compile date.
		if (strlen(GIT_REV) > 5) {
			wintitle.Printf( L"%s-execmon %s", pxGetAppName().c_str(), GIT_REV);
		} else {
			wintitle.Printf( L"%s-execmon  %d.%d.%d-%lld%s (git)", pxGetAppName().c_str(), PCSX2_VersionHi, PCSX2_VersionMid,
